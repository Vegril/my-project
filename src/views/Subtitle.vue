  ...
  <div class="subtitle">
    ...
    <div class="box">
      <div class="optionsBox">
        ...
        <div>
          对齐方式<el-select v-model="applyOptions.textAlign" placeholder="请选择">
          <el-option
        ...
        <div>
          ...
          <span v-show="applyOptions.fontShadow">x偏移量</span><el-slider v-model="applyOptions.fontShadowXOffset" v-show="applyOptions.fontShadow"  style="width:150px;display:inline-block"></el-slider>
          <span v-show="applyOptions.fontShadow">y偏移量</span><el-slider v-model="applyOptions.fontShadowYOffset" v-show="applyOptions.fontShadow"  style="width:150px;display:inline-block"></el-slider>
          <span v-show="applyOptions.fontShadow">投影颜色</span><el-color-picker :show-alpha='true' :color-format="'rgb'" v-model="applyOptions.fontShadowColor" v-show="applyOptions.fontShadow"></el-color-picker>
          <span v-show="applyOptions.fontShadow">投影模糊度</span><el-slider v-model="applyOptions.fontShadowBlur" :max="20" v-show="applyOptions.fontShadow" style="width:150px;display:inline-block"></el-slider>
        </div>
        <div>
          字体大小<el-slider v-model="applyOptions.fontSize" @input="measureWidth" @change="measureWidth" style="width:500px;display:inline-block"></el-slider>
        </div>
        <div>
          开启描边<el-switch v-model="applyOptions.useStroke"></el-switch>
          <span v-show="applyOptions.useStroke">内描边颜色</span>
          <el-color-picker v-show="applyOptions.useStroke" v-model="applyOptions.strokeColor"></el-color-picker>
          <span v-show="applyOptions.useStroke">内描边宽度</span>
          <el-slider v-show="applyOptions.useStroke" v-model="applyOptions.strokeWidth" :max="10" style="width:500px;display:inline-block"></el-slider>
        </div>
        <div v-show="applyOptions.useStroke">
          <span>外描边颜色</span>
          <el-color-picker v-model="applyOptions.strokeColorW"></el-color-picker>
          <span>外描边宽度</span>
          <el-slider v-model="applyOptions.strokeWidthW" :max="10" style="width:500px;display:inline-block"></el-slider>
        </div>
        <div>
          不透明度<el-slider v-model="applyOptions.alpha" style="width:500px;display:inline-block"></el-slider>
        </div>
        <div>
          旋转角度:<el-slider v-model="applyOptions.rotateValue" :max="360" style="width:500px;display:inline-block"></el-slider>
        </div>
        <div>Y轴旋转动画<button @click="changeScale">{{isPlaying?'暂停':'播放'}}</button></div>
...
export default {
  ...
  methods:{
    drawSubtitle:function(){
      ...
      ctx.strokeRect(this.applyOptions.posX,this.applyOptions.posY,this.applyOptions.width,this.applyOptions.fontSize)
      ...
      if(this.applyOptions.rotateValue){
        ctx.save();
        ctx.translate(this.applyOptions.width/2,this.applyOptions.height/2)
      ...
      if(this.applyOptions.useStroke&&this.applyOptions.strokeWidthW!=0){
        ...
        ctx.strokeText(this.content, this.applyOptions.posX,this.applyOptions.posY);
      }
      ...
      ctx.fillText(this.content, this.applyOptions.posX,this.applyOptions.posY);
